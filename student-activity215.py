...
def generate_lidar_blueprint(blueprint_library):
    lidar_blueprint =blueprint_library.find('sensor.lidar.ray_cast_semantic') #locate lidar sensor and store it as data
    lidar_blueprint.set_attribute('channels', str(64))#?
    lidar_blueprint.set_attribute('points_per_second', str(56000*10)) #how many laser beams it emits per second
    lidar_blueprint.set_attribute('rotation_frequency', str(100)) #how fast the beams are moving
    lidar_blueprint.set_attribute('range', str(100.0*200)) #how far it can go
    lidar_blueprint.set_attribute('upper_fov', str(15))
    lidar_blueprint.set_attribute('lower_fov', str(-30))
    return lidar_blueprint


def semantic_lidar_data(point_cloud, lidar_point_cloud_buffer): #it will sort the numbers in the console based on x, y, and z
...
def carlaThreadingLoop(world): #every 0.005 milliseconds it will collect new data and help it understand the data
    frame = 0
    while True:
        time.sleep(0.005)
        world.tick()
...
try:
    ...
    visualise_lidar_using_mayavi = mlab.points3d(10, 10, 10, 10, mode='point', figure=lidar_figure, mode='point', colormap='gnuplot')
    ...
    sensor.listen(lambda data: semantic_lidar_data(data, lidar_point_cloud_buffer)) #it will store everything in data variable
